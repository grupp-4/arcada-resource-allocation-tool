...
export default function wcLib(wc, options) {
    ...
    return {
        ...
        async setHoursForCourse(courseName, teacherIndex, period, hours) {
            ...
            if (!(await wc.courses.update(courseName, {[path]: hours}))) {
                throw new Error(`Failed updating ${path} for course ${courseName}!`)
            }
        ...
        async setHoursForTeacher(teacherName, courseIndex, period, hours) {
            ...
            if (!(await wc.teachers.update(teacherName, {[path]: hours}))){
                throw new Error(`Failed updating ${path} for teacher ${teacherName}!`)
            }
        ...
        async setTeacherForCourse(courseName, teacherName, teacherIndex) {
            ...
            if (!(await wc.courses.update(courseName, {[path]: {name: teacherName, hours: [0, 0, 0, 0]}}))) {
                throw new Error(`Failed assigning ${teacherName} to ${courseName}!`)
            }
        ...
        async setCourseForTeacher(teacherName, courseName, courseIndex) {
            ...
            if (!(await wc.teachers.update(teacherName, {[path]: {name: courseName, hours: [0, 0, 0, 0]}}))) {
                throw new Error(`Failed assigning ${courseName} to ${teacherName}!`)
            }
