    ...
    if (page) {
        switch (page) {
            case "courses":
                ...
                return <Courses db={db} mobile={mobile} strings={strings} loglevel={log.getLevel()}/>
            case "teachers":
                log.info("Loading page", page)
                return <Teachers db={db} mobile={mobile} strings={strings} loglevel={log.getLevel()}/>
            case "events-feed":
                if (mobile) {
                    log.info("Loading page", page)
                    return <EventsFeed db={db} mobile={mobile} strings={strings} loglevel={log.getLevel()}/>
                }
                break
            default:
        }
